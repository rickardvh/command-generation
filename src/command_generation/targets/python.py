...
def _python_primitive_executor_module(
    *,
    source_path: str,
    regenerate_command: str,
    host_manifest: CommandGenerationHostManifest,
...
def _python_local_runtime_helper_block() -> str:
    return (
        ...
        "    def limit_error(*, reason: str, selector_index: int | None = None, selector_length: int | None = None) -> dict[str, Any]:\n"
        "        error: dict[str, Any] = {'reason': reason, 'requested_selector_count': requested_selector_count, 'max_selectors': _MAX_PROJECTION_SELECTORS, 'max_selector_length': _MAX_PROJECTION_SELECTOR_LENGTH}\n"
        ...
        "            return limit_error(reason='too-many-selectors', selector_index=requested_selector_count - 1)\n"
        ...
        "            requested_selector_count += 1\n"
        "            error = limit_error(reason='selector-too-long', selector_index=requested_selector_count - 1, selector_length=len(token_chars))\n"
        "            return {'selectors': selectors, 'error': error}\n"
        "    error = append_selector()\n"
        "    return {'selectors': selectors, 'error': error}\n\n\n"
        ...
        "        return selected_output_kind.replace('/selected-output/', '/selector-validation-error/', 1)\n"
...
def _python_local_runtime_generated_function(
    ...
    *,
    ...
    if implementation == "sectioned_payload_select":
        ...
        selector_inventory_command = str(
            override.get("selector_inventory_command") or ""
        )
        selector_detail_command = str(override.get("selector_detail_command") or "")
        ...
        if (
            ...
            outputs.append(
                GeneratedOutput(
                    ...
                    _python_primitive_executor_module(
                        source_path=source_path,
                        regenerate_command=regenerate_command,
                        host_manifest=host_manifest,
                    ),
